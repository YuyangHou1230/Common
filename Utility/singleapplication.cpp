...
#else
SingleApplication::SingleApplication(int &argc, char **argv, int flags)
...
#ifdef Q_QDOC
SingleApplicationByLockFile::SingleApplicationByLockFile(int &argc, char **argv)
    : QApplication(argc, argv)
{
    m_lockFilePath = applicationDirPath();
}
#else
SingleApplicationByLockFile::SingleApplicationByLockFile(int &argc, char **argv, int flags)
    : QApplication(argc, argv, flags)
{
    m_lockFilePath = applicationDirPath();
}
...
#else
SingleApplicationSharedMemory::SingleApplicationSharedMemory(int &argc, char **argv, int flags)
    : QApplication(argc, argv, flags)
{

    QSharedMemory    sharedMemory("MySharedMemory");
    QSystemSemaphore semaphore("MySemaphore", 0);

    if ( !sharedMemory.create(128) )
    {
        if ( !sharedMemory.attach() )
        {
            qDebug() << "Error attaching to shared memory:" << sharedMemory.errorString();
        }
    }

    char *      data    = static_cast<char *>(sharedMemory.data());
    const char *message = "Hello from Process 1!";
    qstrncpy(data, message, qstrlen(message));

    // 通知进程2有新数据
    semaphore.release();
}

bool SingleApplicationSharedMemory::isRunning()
{
}

void SingleApplicationSharedMemory::doSomethingIfIsRun()
{
}
#endif
